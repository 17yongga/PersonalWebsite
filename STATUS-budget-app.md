...
- **App Store update preparation:** build 17 was uploaded but used already-live version `1.0.1`, so a proper App Store update needs `1.0.2`. Prepared and synced App Store Connect metadata for `1.0.2`, including release notes for the settlement/backend-source-of-truth fix. EAS iOS production build `5e83d0e1-b1b5-4ffe-b7ad-38592ada3d7d` is queued/running as version `1.0.2` build `18`; cron watcher `b08bdf228aa5` will upload it to App Store Connect/TestFlight when finished, but will not submit for App Review. Gary must manually review before final Submit for Review.
