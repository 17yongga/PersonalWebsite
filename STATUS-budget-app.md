...
- **Flowt Assistant MVP backend is live in guarded fallback mode:** `POST /api/ai/chat` is deployed to production behind JWT auth and household scoping. Production currently has no `OPENAI_API_KEY`, so the route returns the deterministic read-only fallback (`providerStatus: fallback_no_api_key`) instead of calling a paid model. This keeps costs at $0 until we explicitly configure the provider key/quota.
...
- **Verification:** production backend was backed up to `/home/ubuntu/budget-server/backups/deploy-20260604-180556/`, syntax checks passed, remote `npm test` passed, PM2 `budget-server` was restarted only, live health returned 200, unauthenticated `/api/ai/chat` returned 401, and authenticated localhost smoke returned 200 with `mode: read_only`, 3 cards, and fallback provider status. Local mobile `npm run test:ui-logic` passes 21/21; `npm run capture:assistant` passes and generated assistant artifacts: `flowt-app/artifacts/flowt-1.0.3-assistant-dashboard-entry.png`, `flowt-app/artifacts/flowt-1.0.3-assistant-answer.png`, and `flowt-app/artifacts/flowt-1.0.3-assistant-cards.png`.
