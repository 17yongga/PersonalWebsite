...
- **Flowt Assistant MVP started locally:** added the implementation plan plus a read-only, security/cost-controlled assistant foundation. Mobile Dashboard now has a top-right sparkle entry and `FlowtAssistantModal`; backend has `POST /api/ai/chat` with JWT auth, household scoping, compact context generation, OpenAI low-cost provider wrapper, deterministic fallback when no API key is configured, usage/quota metadata, and no full transcript storage.
- **Local verification:** backend `npm test` passes 25/25 including assistant context/prompt/cost tests; mobile `npm run test:ui-logic` passes 21/21 including assistant response helpers. `npm run capture:fixes`, `npm run capture:journeys`, `npm run capture:budget-settings`, and new `npm run capture:assistant` pass against mocked local API data. New assistant artifacts include `flowt-app/artifacts/flowt-1.0.3-assistant-dashboard-entry.png`, `flowt-app/artifacts/flowt-1.0.3-assistant-answer.png`, and `flowt-app/artifacts/flowt-1.0.3-assistant-cards.png`.
