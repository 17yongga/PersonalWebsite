...
- **Flowt iOS TestFlight 1.0.4 build 22 queued with latest local fixes and auto-submit:** After Gary confirmed local fixing was done, release gates were rerun against commit `7a39cd3` (`fix(flowt): clarify shared balance settlement state`). Verification passed: production backend health **200**, app `npm run test:ui-logic` **119/119**, filtered TypeScript **0 non-Remotion errors** (only known `marketing/remotion-demo` missing `remotion`), EAS metadata lint valid, Expo config shows version `1.0.4` and bundle ID `com.garyyong.flowt`, and focused Playwright visual QA passed for settled + owed Shared Balance states. Build 21 is superseded by this newer build. EAS incremented iOS build number **21 → 22** and queued production build **d5b40c7e-8d63-45ee-b95a-194654f6798e** with auto-submit; submission **5b382514-7e3a-4870-9a61-f2449d09ebb1**. Watcher cron `02d00ebb5ebe` checks every 15 minutes and will report when it finishes/fails. This schedules TestFlight/App Store Connect processing only — no final App Review submission was performed.
