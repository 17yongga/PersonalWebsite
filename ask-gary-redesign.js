...
function showLoading() {
  ...
  loadingMsg.innerHTML = `
    <div class="thinking-indicator">
      <span>Ask Gary is thinking</span>
...
async function sendMessage() {
  ...
  try {
    ...
    const errorMessage = `Ask Gary hit an error: ${error.message}. Please try again shortly or contact Gary directly if the issue continues.`;
...
userInput.addEventListener("input", () => {
  ...
  userInput.style.height = `${Math.min(userInput.scrollHeight, 104)}px`;
});
...
// Focus only on larger screens so mobile visitors start at the top of the page.
