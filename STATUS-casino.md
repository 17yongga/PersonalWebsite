...
## Verified Pachinko Mobile Controls Candidate — `neon777-20260803-r56` (Awaiting Promotion)
- **Reported mobile defect fixed:** the Risk and Balls controls no longer wrap into awkward 2+1 half-width cards. Mobile now follows one intentional hierarchy: Bet Per Ball, four quick amounts, a full-width three-button Risk row, a full-width three-button Balls row, then the primary Drop action. Nested spacing and panel weight were reduced without changing the Pachinko board, payout presentation, or desktop workspace.
- **Interaction semantics:** quick amounts, manual input, Risk, and Balls keep exactly one observable selection through synchronized `.active` and `aria-pressed` state. Click, physical touch, and keyboard Space were exercised. Manual non-preset input clears false quick-selection state, and selector actions produced zero API mutations.
- **Verification:** source suite passed **97/97**. Exact package passed **72/72** core responsive views, focused Pachinko at 375×667, 390×844, 393×852, and 430×932, 844×390 landscape, reduced motion at 390×844, zero-growth lifecycle re-entry, and Roulette/audio regression. Exact 393px and 1280px screenshots passed visual review; the generic desktop scanner's two known header-text flags were visually false positives unrelated to Pachinko.
- **Immutable package:** Linux x86_64 `r56` contains 6,282 manifested files with manifest SHA-256 `ddd2606dc0f74ec1d7d203b690daeb0d10f31c37db66836ad4bc5a2c56d5d9de`, entry SHA-256 `6ed7ed048589db2e366b471064564bb7fb6961b4e12e2b48b3228f28e7ee6e22`, 25/25 runtime references targeting `r56`, exact changed-source parity, zero symlinks/world-writable paths, and a valid Linux `better-sqlite3` binary.
- **Production unchanged:** frontend/backend remain immutable `r55`. No production wager, account mutation, deployment, restart, invalidation, or push was performed. Exact `r56` promotion requires Gary's separate approval.
